    ...
    def execute(self):
        if not self.hasTable("tv_shows") and not self.hasTable("db_version"):
            queries = [
                ...
                "INSERT INTO db_version (db_version) VALUES (32);"
            ]
...
class AddTorrentHash(AddLastUpdateTVDB):
...
class AddLastProperSearch(AddLastDownloadableSearch):
    def test(self):
...
class AddDvdOrderOption(AddRequireAndIgnoreWords):
    def test(self):
...
class ConvertTVShowsToIndexerScheme(AddDvdOrderOption):
	...
	backupDatabase(self.checkDBVersion())
        ...
        self.connection.action("ALTER TABLE tv_shows RENAME TO tmp_tv_shows")
        self.connection.action(
            "CREATE TABLE tv_shows (show_id INTEGER PRIMARY KEY, indexer_id NUMERIC, indexer NUMERIC, show_name TEXT, location TEXT, network TEXT, genre TEXT, classification TEXT, runtime NUMERIC, quality NUMERIC, airs TEXT, status TEXT, flatten_folders NUMERIC, paused NUMERIC, startyear NUMERIC, air_by_date NUMERIC, lang TEXT, subtitles NUMERIC, notify_list TEXT, imdb_id TEXT, last_update_indexer NUMERIC, dvdorder NUMERIC, rls_require_words TEXT, rls_ignore_words TEXT)")
        self.connection.action(
...
class AddSportsOption(ConvertIndexerToInteger):
    def test(self):
